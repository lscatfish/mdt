/**
 * Anchored tool bootstrap (open variant) — keep the FIRST model request on the
 * official Minimal preset's REAL tool schema (persistent `bash` +
 * `str_replace_editor`), free of auto-injected workspace/skill context, then
 * expose the FULL assembled catalog once the session has produced its first
 * durable promotion signal.
 *
 * This is the "open" counterpart of upstream's resident-set variant: it keeps
 * the same first-request anchor (issue #11: the Minimal pair anchored 5/5 at
 * the adapter-default maxTokens while every standard-family schema fell into
 * standard-like behavior 11/11) but, after promotion, returns the complete
 * catalog unchanged instead of narrowing to a resident set. No dev_tool_search
 * indirection: every Standard tool is visible from request #2 on, and the
 * auto-injected context (AGENTS.md digest, skill catalog) returns with it.
 *
 ...
 * full catalog. The original `'tool-call'` mode is kept for compatibility,
 ...
 * promotion signal exists past that boundary.
 *
 * Robustness:
 ...
 * the Minimal tool schema anchors at the adapter-default maxTokens, so the
 * cap is opt-in rather than the default.
 */
...
export function apply(ctx, config) {
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    try {
      ...
      if (status.promoted) {
        // PROMOTED: expose the FULL assembled catalog unchanged. The anchor
        // only constrains request #1; from request #2 the model sees every
        // Standard tool plus the restored context injections.
        return assembled
      }
